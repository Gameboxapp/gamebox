...
paises = ["ARGENTINA", "BRASIL", "ITALIA", "ESPAÑA", "CHILE", "ECUADOR", "GUATEMALA", "MEXICO", "COLOMBIA", "NICARAGUA","COSTARICA"];
...
function desordenarPaises(){
    for(var i=0;i<paises.length;i++){
        ...
        //Guardamos el pais en el arreglo de paises desordenados
...
function move() {
  if (x == 0) {
    ...
    let width = 1;
    idInterval = setInterval(frame, 100); // Cambia este valor para hacer el juego más lento
    function frame() {
      if (width >= 100) {
        clearInterval(idInterval);
        x = 0;
